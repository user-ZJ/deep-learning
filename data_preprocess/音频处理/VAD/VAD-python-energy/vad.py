...
class VoiceActivityDetector():
    ...
    def __init__(self, wave_input_filename):
        ...
        self.sample_overlap = 0.01 #10ms 窗口滑动
    ...
    def detect_speech(self):
        ...
        sample_window = int(self.rate * self.sample_window) # 采样率*窗口大小 8000*0.02
        sample_overlap = int(self.rate * self.sample_overlap) # 8000*0.01
        data = self.data
        sample_start = 0
    ...
    def save_wav(self):
        ...
        while not it.finished:
            ...
            data_speech[int(it[0])] = data[int(it[0])] #* detected_windows[it.index, 1]
            it.iternext()
        for frame,slience in detected_windows:
            VADdata.append(data[int(frame):int(frame+79)]*slience)
        VADdata = np.array(VADdata).flatten()
        print(len(VADdata),data.shape,data_speech.shape)
        plt.figure()
        plt.plot(VADdata)
        plt.show()
        wf.write("VAD.wav",8000,VADdata)
