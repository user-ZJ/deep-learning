...
with tf.Session() as sess:
    ...
    while iter<100:
        ...
        filename = saver.save(sess, "models/model.ckpt")

        iter=iter+1

        graph_def = tf.get_default_graph().as_graph_def()
        output_graph_def = tf.graph_util.convert_variables_to_constants(sess, graph_def, ["output"])
        with tf.gfile.GFile('models/lstm_frozen.pb', "wb") as f:
...
# convert to tflite
#bazel run --config=opt tensorflow/lite/toco:toco -- --input_file=/home/zack/project/voice_recognation/lstm_frozen.pb  \
# --input_format=TENSORFLOW_GRAPHDEF --output_format=TFLITE --output_file=/home/zack/project/voice_recognation/lstm_frozen.tflite \
# --input_arrays="Placeholder" --input_shapes=1,28,28 --output_arrays="output" --allow_custom_ops