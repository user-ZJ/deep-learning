...
class SampleTFRecordCreator(object):
    ...
    def create_datainfo(self):
        ...
        datainfo['data_shape'] = feature[0].shape #[context,mel]
        datainfo['num_labels'] = self.num_labels
        datainfo['label_dict'] = self.label_dict
        datainfo['train_files_num'] = len(self.train_files)
        datainfo['validation_files_num'] = len(self.validation_files)
        datainfo['test_files_num'] = len(self.test_files)
        datainfo['train_files'] = [self.id_filename_dict[id] for id in self.train_files]
        datainfo['validation_files'] = [self.id_filename_dict[id] for id in self.validation_files]
        datainfo['test_files'] = [self.id_filename_dict[id] for id in self.test_files]
