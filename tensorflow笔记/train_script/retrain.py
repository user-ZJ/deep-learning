...
def run_final_eval(train_session, module_spec, class_count, image_lists,
  ...
  Args:
    ...
    bottleneck_tensor: The bottleneck output layer of the CNN graph.
  """
  # test_bottlenecks 测试集缓存文件列表[batch_size]
  # test_ground_truth 测试集标签[batch_size]
  # test_filenames 测试集文件名列表[batch_size]
  ...
  # eval_session 训练完成后的网络
  # bottleneck_input  测试集缓存文件  输入
  # ground_truth_input 测试集标签 输入
  # evaluation_step 准确率
  # prediction 预测的类别
...
def main(_):
  ...
  # 读取文件，并返回字典类型文件名列表
  # result[label_name] = {
  #     'dir': dir_name,   目录名，对应为标签名
  #     'training': training_images,  训练集图片列表
  #     'testing': testing_images,  测试集图片列表
  #     'validation': validation_images, 验证集图片列表
  # }
  ...
  # graph pre-train网络结构
  # bottleneck_tensor pre-train网络输出[batch_size,1280]
  # resized_image_tensor pre-train网络输入[batch_size,224,224,3]
  # wants_quantization 是否做量化
  ...
  with graph.as_default():
      ...
      # train_step : optimizer.minimize(cross_entropy_mean)
      # cross_entropy:交叉熵
      # bottleneck_input：[batch_size,1028] 输入  每个batch中对应图片的缓存文件内容
      # ground_truth_input:[batch_size] 输入  每个batch中图片对应的label
      # final_tensor：[batch_size,class_count]  每个batch中图片被预测为各个类别的概率
    (train_step, cross_entropy, bottleneck_input,
     ground_truth_input, final_tensor) = add_final_retrain_ops(
         class_count, FLAGS.final_tensor_name, bottleneck_tensor,
         wants_quantization, is_training=True)

  # 使用添加了全连接层的网络来初始化session
  with tf.Session(graph=graph) as sess:
    ...
    # jpeg_data_tensor:string 图片名  输入
    # decoded_image_tensor：resize后[batch_size,224,224,3]的tensor
    ...
    if do_distort_images:
      ...
      # cached them on disk.
      # 将图片解码后，resize为[batch_size,224,224,3],通过预训练网络，计算为[1280] float类型，缓存到文件
      # jpeg_data_tensor string 图片名
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if do_distort_images:
        (train_bottlenecks,
         train_ground_truth) = get_random_distorted_bottlenecks(
          ...
          #train_bottleneck：缓存的图片经过预训练网络后的[1280]数据文件名列表
          # train_ground_truth:图片对应的标签
        (train_bottlenecks,
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
          # evaluation_step 每个batch 准确率
          # cross_entropy 全连接层交叉熵
        ...
        # practice this makes a negligable difference.
          # validation_bottlenecks 验证集图片对应的[1280]缓存文件
          # validation_ground_truth 图片对应的标签
          # 随机选取validation_batch_size张图片
        ...
        # with the `merged` op.
          # validation_accuracy验证集准确率
          # validation_bottlenecks 验证集输入缓存文件 [batch_size,1280]
          # validation_ground_truth 验证集标签[batch_size]
